...
const typeNum = (arr) => {
  let newNum = arr.filter(num => (typeof(num) === 'number'));
  return newNum;
};
...
const containsAnd = (arr) => {
  let and = arr.filter(arr => arr.includes('and'));
  return and;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let filteredWord = arr.filter((word) => !forbiddenValues.includes(word));
  return filteredWord;
};
